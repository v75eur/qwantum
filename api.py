...
def log(m, msg):
    ...
    with LOCKS[m]:
        LOGS[m].append(line)
        if len(LOGS[m]) > 500: LOGS[m].pop(0)

FILES = {
    "np": "dns_np.csv", "gt": "dns_gt.csv", "y@": "dns_y@.csv"
}
...
def download(m):
    ...
    emails = read_csv(FILES[m])
    return jsonify({"emails": emails[:n]})
